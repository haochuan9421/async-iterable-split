...
export class NeedleIterator extends SubIterator {
  ...
  constructor(spliter: Spliter, needle: Uint8Array) {
    super(spliter);
  ...
  async next(): Promise<IteratorResult<Uint8Array>> {
    ...
    if (index !== -1) {
      // 如果匹配成功了，模式串前面的内容返回，后面的内容转移到 spliter.remain 以备后用
      this.end();
      this.spliter.remain = haystack.subarray(index + this.needle.length);
      return { done: false, value: haystack.subarray(0, index) };
    }

    // 如果匹配失败了，则此时的 offset 是之后匹配的起点，offset 之前的内容是必然无法匹配的，可以直接返回了，之后的内容转移到 spliter.remain 以备后用
    this.spliter.remain = haystack.subarray(offset);
    return { done: false, value: haystack.subarray(0, offset) };
  }

  async fetchHaystack(least: number): Promise<Uint8Array> {
    const item = await this.spliter.next();
    if (item.done) {
    ...
    while (chunksLen < least) {
      const moreItem = await this.spliter.next();
      if (moreItem.done) {
        break;
      }
